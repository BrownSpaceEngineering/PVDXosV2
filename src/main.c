...
flags and correct compiler versions If you want to get rid of the red squiggly
lines:
...
#error \
    "Multiple build type flags set! (DEVBUILD && UNITTEST) Must be exactly one of: {DEVBUILD, UNITTEST, RELEASE}"
#endif
#if defined(DEVBUILD) && defined(RELEASE)
#error \
    "Multiple build type flags set! (DEVBUILD && RELEASE) Must be exactly one of: {DEVBUILD, UNITTEST, RELEASE}"
#endif
#if defined(UNITTEST) && defined(RELEASE)
#error \
    "Multiple build type flags set! (UNITTEST && RELEASE) Must be exactly one of: {DEVBUILD, UNITTEST, RELEASE}"
#endif
...
int main(void) {
    ...
    // The heartbeat task is a simple task that blinks the LEDs in a pattern to
    // indicate that the system is running xTaskCreateStatic(main_func,
    // "TaskName", StackSize, pvParameters, Priority, StackBuffer, TaskTCB);
    TaskHandle_t heartbeatTaskHandle = xTaskCreateStatic(
        heartbeat_main, "Heartbeat", HEARTBEAT_TASK_STACK_SIZE, NULL, 1,
