...
/*
 * Do LU-factorization of a matrix A, size row x column, and get the L matrix, size row x column,
...
void lu(float* A, float* L, float* U, float* P, int row, int column) {
    ...
    // Solve!
    dgetrf_(&M, &N, A_, &LDA, IPIV, &INFO_LU,
            IPIV_d); // I've change the dgetrf so it will return a IPIV vector of floats, due to
                     // memory loss.
