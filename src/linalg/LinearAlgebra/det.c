...
float det(float* A, int row) {
    ...
    if (row == 1) {
        ...
        return (*(A + 0)) * (*(A + 4)) * (*(A + 8)) + (*(A + 1)) * (*(A + 5)) * (*(A + 6))
               + (*(A + 2)) * (*(A + 3)) * (*(A + 7)) -

               (*(A + 6)) * (*(A + 4)) * (*(A + 2)) - (*(A + 7)) * (*(A + 5)) * (*(A + 0))
               - (*(A + 8)) * (*(A + 3)) * (*(A + 1));
...
// TODO probably a bad idea. We need to figure out whether we care about exact equality to 0
// or an underflow error here.
