...
/* Subroutine */ int slaqr5_(logfical* wantt, logfical* wantz, integer* kacc22, integer* n,
    ...
    logfical accum;
    integer ndcol, incol;
    extern /* Subroutine */ int sgemm_(char*, char*, integer*, integer*, integer*, real*, real*,
                                       integer*, real*, integer*, real*, real*, integer*);
    integer krcol, nbmps;
    extern /* Subroutine */ int strmm_(char*, char*, char*, char*, integer*, integer*, real*, real*,
                                       integer*, real*, integer*),
        slaqr1_(integer*, real*, integer*, real*, real*, real*, real*, real*),
        slabad_(real*, real*);
    extern floatreal slamch_(char*);
    real safmin;
    extern /* Subroutine */ int slarfg_(integer*, real*, real*, integer*, real*);
    real safmax;
    extern /* Subroutine */ int slacpy_(char*, integer*, integer*, real*, integer*, real*,
                                        integer*),
        ...
        for (krcol = incol; krcol <= i__3; ++krcol) {
            ...
            for (m = mtop; m <= i__4; ++m) {
                ...
                if (k == *ktop - 1) {
                    ...
                    if (h__[k + 3 + k * h_dim1] != 0.f || h__[k + 3 + (k + 1) * h_dim1] != 0.f
                        ...
                        if ((r__1 = h__[k + 2 + k * h_dim1] - refsum * vt[1], dfabs(r__1))
                                + (r__2 = refsum * vt[2], dfabs(r__2))
                            > ulp
                                  * ((r__3 = h__[k + k * h_dim1], dfabs(r__3))
                                     + (r__4 = h__[k + 1 + (k + 1) * h_dim1], dfabs(r__4))
                                     + (r__5 = h__[k + 2 + (k + 2) * h_dim1], dfabs(r__5)))) {
            ...
            for (m = mstart; m <= i__4; ++m) {
                ...
                if (h__[k + 1 + k * h_dim1] != 0.f) {
                    tst1 = (r__1 = h__[k + k * h_dim1], dfabs(r__1))
                           + (r__2 = h__[k + 1 + (k + 1) * h_dim1], dfabs(r__2));
                    if (tst1 == 0.f) {
                        if (k >= *ktop + 1) {
                            tst1 += (r__1 = h__[k + (k - 1) * h_dim1], dfabs(r__1));
                        }
                        if (k >= *ktop + 2) {
                            tst1 += (r__1 = h__[k + (k - 2) * h_dim1], dfabs(r__1));
                        }
                        if (k >= *ktop + 3) {
                            tst1 += (r__1 = h__[k + (k - 3) * h_dim1], dfabs(r__1));
                        }
                        if (k <= *kbot - 2) {
                            tst1 += (r__1 = h__[k + 2 + (k + 1) * h_dim1], dfabs(r__1));
                        }
                        if (k <= *kbot - 3) {
                            tst1 += (r__1 = h__[k + 3 + (k + 1) * h_dim1], dfabs(r__1));
                        }
                        if (k <= *kbot - 4) {
                            tst1 += (r__1 = h__[k + 4 + (k + 1) * h_dim1], dfabs(r__1));
                        }
                    }
                    /* Computing MAX */
                    r__2 = smlnum, r__3 = ulp * tst1;
                    if ((r__1 = h__[k + 1 + k * h_dim1], dfabs(r__1)) <= dmax(r__2, r__3)) {
                        /* Computing MAX */
                        r__3 = (r__1 = h__[k + 1 + k * h_dim1], dfabs(r__1)),
                        r__4 = (r__2 = h__[k + (k + 1) * h_dim1], dfabs(r__2));
                        h12 = dmax(r__3, r__4);
                        /* Computing MIN */
                        r__3 = (r__1 = h__[k + 1 + k * h_dim1], dfabs(r__1)),
                        r__4 = (r__2 = h__[k + (k + 1) * h_dim1], dfabs(r__2));
                        h21 = dmin(r__3, r__4);
                        /* Computing MAX */
                        r__3 = (r__1 = h__[k + 1 + (k + 1) * h_dim1], dfabs(r__1)),
                        ...
                        if (tst2 == 0.f || h21 * (h12 / scl) <= dmax(r__1, r__2)) {
        ...
        if (accum) {
            ...
            if (!blk22 || incol < *ktop || ndcol > *kbot || ns <= 2) {
                ...
                for (jcol = min(ndcol, *kbot) + 1; i__4 < 0 ? jcol >= i__3 : jcol <= i__3;
                    ...
                    slacpy_("ALL", &nu, &jlen, &wh[wh_offset], ldwh,
                ...
                for (jrow = jtop; i__3 < 0 ? jrow >= i__4 : jrow <= i__4; jrow += i__3) {
                    ...
                    jlen = min(i__5, i__7);
                    sgemm_("N", "N", &jlen, &nu, &nu, &c_b8, &h__[jrow + (incol + k1) * h_dim1],
                ...
                for (jcol = min(ndcol, *kbot) + 1; i__3 < 0 ? jcol >= i__4 : jcol <= i__4;
                    ...
                    slacpy_("ALL", &knz, &jlen, &h__[incol + 1 + j2 + jcol * h_dim1], ldh,
                            &wh[kzs + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply by U21' ==== */

                    slaset_("ALL", &kzs, &jlen, &c_b7, &c_b7, &wh[wh_offset], ldwh);
                    strmm_("L", "U", "C", "N", &knz, &jlen, &c_b8, &u[j2 + 1 + (kzs + 1) * u_dim1],
                           ldu, &wh[kzs + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply top of H by U11' ==== */

                    sgemm_("C", "N", &i2, &jlen, &j2, &c_b8, &u[u_offset], ldu,
                           &h__[incol + 1 + jcol * h_dim1], ldh, &c_b8, &wh[wh_offset], ldwh);

                    /*                 ==== Copy top of H to bottom of WH ==== */

                    slacpy_("ALL", &j2, &jlen, &h__[incol + 1 + jcol * h_dim1], ldh,
                            &wh[i2 + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply by U21' ==== */

                    strmm_("L", "L", "C", "N", &j2, &jlen, &c_b8, &u[(i2 + 1) * u_dim1 + 1], ldu,
                           &wh[i2 + 1 + wh_dim1], ldwh);
                    ...
                    i__5 = i4 - i2;
                    i__7 = j4 - j2;
                    sgemm_("C", "N", &i__5, &jlen, &i__7, &c_b8, &u[j2 + 1 + (i2 + 1) * u_dim1],
                           ldu, &h__[incol + 1 + j2 + jcol * h_dim1], ldh, &c_b8,
                           &wh[i2 + 1 + wh_dim1], ldwh);

                    /*                 ==== Copy it back ==== */

                    slacpy_("ALL", &kdu, &jlen, &wh[wh_offset], ldwh,
                ...
                for (jrow = jtop; i__4 < 0 ? jrow >= i__3 : jrow <= i__3; jrow += i__4) {
                    ...
                    slacpy_("ALL", &jlen, &knz, &h__[jrow + (incol + 1 + j2) * h_dim1], ldh,
                            &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U21 ==== */

                    slaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[wv_offset], ldwv);
                    strmm_("R", "U", "N", "N", &jlen, &knz, &c_b8, &u[j2 + 1 + (kzs + 1) * u_dim1],
                           ldu, &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U11 ==== */

                    sgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &h__[jrow + (incol + 1) * h_dim1], ldh,
                           &u[u_offset], ldu, &c_b8, &wv[wv_offset], ldwv);

                    /*                 ==== Copy left of H to right of scratch ==== */

                    slacpy_("ALL", &jlen, &j2, &h__[jrow + (incol + 1) * h_dim1], ldh,
                            &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U21 ==== */

                    i__5 = i4 - i2;
                    strmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(i2 + 1) * u_dim1 + 1], ldu,
                           &wv[(i2 + 1) * wv_dim1 + 1], ldwv);
                    ...
                    i__5 = i4 - i2;
                    i__7 = j4 - j2;
                    sgemm_("N", "N", &jlen, &i__5, &i__7, &c_b8,
                ...
                if (*wantz) {
                    ...
                    for (jrow = *iloz; i__3 < 0 ? jrow >= i__4 : jrow <= i__4; jrow += i__3) {
                        ...
                        slacpy_("ALL", &jlen, &knz, &z__[jrow + (incol + 1 + j2) * z_dim1], ldz,
                                &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                        /*                    ==== Multiply by U12 ==== */

                        slaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[wv_offset], ldwv);
                        strmm_("R", "U", "N", "N", &jlen, &knz, &c_b8,
                               &u[j2 + 1 + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) * wv_dim1 + 1],
                               ldwv);

                        /*                    ==== Multiply by U11 ==== */

                        sgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &z__[jrow + (incol + 1) * z_dim1],
                               ldz, &u[u_offset], ldu, &c_b8, &wv[wv_offset], ldwv);

                        /*                    ==== Copy left of Z to right of scratch ==== */

                        slacpy_("ALL", &jlen, &j2, &z__[jrow + (incol + 1) * z_dim1], ldz,
                                &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

                        /*                    ==== Multiply by U21 ==== */

                        i__5 = i4 - i2;
                        strmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(i2 + 1) * u_dim1 + 1],
