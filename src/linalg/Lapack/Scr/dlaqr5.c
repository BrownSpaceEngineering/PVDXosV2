...
/* Subroutine */ int dlaqr5_(logfical* wantt, logfical* wantz, integer* kacc22, integer* n,
    ...
    extern /* Subroutine */ int dtrmm_(char*, char*, char*, char*, integer*, integer*, floatreal*,
                                       floatreal*, integer*, floatreal*, integer*),
        dlaqr1_(integer*, floatreal*, integer*, floatreal*, floatreal*, floatreal*, floatreal*,
                floatreal*),
        dlabad_(floatreal*, floatreal*);
    extern floatreal dlamch_(char*);
    extern /* Subroutine */ int dlarfg_(integer*, floatreal*, floatreal*, integer*, floatreal*),
        dlacpy_(char*, integer*, integer*, floatreal*, integer*, floatreal*, integer*);
    floatreal safmin;
    extern /* Subroutine */ int dlaset_(char*, integer*, integer*, floatreal*, floatreal*,
                                        floatreal*, integer*);
    ...
    /*      WANTT  (input) logfical scalar */
    /*             WANTT = .true. if the quasi-triangular Schur factor */
    /*             is being computed.  WANTT is set to .false. otherwise. */

    /*      WANTZ  (input) logfical scalar */
        ...
        for (krcol = incol; krcol <= i__3; ++krcol) {
            ...
            for (m = mtop; m <= i__4; ++m) {
                ...
                if (k == *ktop - 1) {
                    ...
                    if (h__[k + 3 + k * h_dim1] != 0. || h__[k + 3 + (k + 1) * h_dim1] != 0.
                        ...
                        if ((d__1 = h__[k + 2 + k * h_dim1] - refsum * vt[1], fabs(d__1))
                                + (d__2 = refsum * vt[2], fabs(d__2))
                            > ulp
                                  * ((d__3 = h__[k + k * h_dim1], fabs(d__3))
                                     + (d__4 = h__[k + 1 + (k + 1) * h_dim1], fabs(d__4))
                                     + (d__5 = h__[k + 2 + (k + 2) * h_dim1], fabs(d__5)))) {
            ...
            for (j = max(*ktop, krcol); j <= i__4; ++j) {
                ...
                for (m = mtop; m <= i__5; ++m) {
                    ...
                    refsum
                        = v[m * v_dim1 + 1]
                          * (h__[k + 1 + j * h_dim1] + v[m * v_dim1 + 2] * h__[k + 2 + j * h_dim1]
            ...
            for (m = mstart; m <= i__4; ++m) {
                ...
                if (h__[k + 1 + k * h_dim1] != 0.) {
                    tst1 = (d__1 = h__[k + k * h_dim1], fabs(d__1))
                           + (d__2 = h__[k + 1 + (k + 1) * h_dim1], fabs(d__2));
                    if (tst1 == 0.) {
                        if (k >= *ktop + 1) {
                            tst1 += (d__1 = h__[k + (k - 1) * h_dim1], fabs(d__1));
                        }
                        if (k >= *ktop + 2) {
                            tst1 += (d__1 = h__[k + (k - 2) * h_dim1], fabs(d__1));
                        }
                        if (k >= *ktop + 3) {
                            tst1 += (d__1 = h__[k + (k - 3) * h_dim1], fabs(d__1));
                        }
                        if (k <= *kbot - 2) {
                            tst1 += (d__1 = h__[k + 2 + (k + 1) * h_dim1], fabs(d__1));
                        }
                        if (k <= *kbot - 3) {
                            tst1 += (d__1 = h__[k + 3 + (k + 1) * h_dim1], fabs(d__1));
                        }
                        if (k <= *kbot - 4) {
                            tst1 += (d__1 = h__[k + 4 + (k + 1) * h_dim1], fabs(d__1));
                        }
                    }
                    /* Computing MAX */
                    d__2 = smlnum, d__3 = ulp * tst1;
                    if ((d__1 = h__[k + 1 + k * h_dim1], fabs(d__1)) <= max(d__2, d__3)) {
                        /* Computing MAX */
                        d__3 = (d__1 = h__[k + 1 + k * h_dim1], fabs(d__1)),
                        d__4 = (d__2 = h__[k + (k + 1) * h_dim1], fabs(d__2));
                        h12 = max(d__3, d__4);
                        /* Computing MIN */
                        d__3 = (d__1 = h__[k + 1 + k * h_dim1], fabs(d__1)),
                        d__4 = (d__2 = h__[k + (k + 1) * h_dim1], fabs(d__2));
                        h21 = min(d__3, d__4);
                        /* Computing MAX */
                        d__3 = (d__1 = h__[k + 1 + (k + 1) * h_dim1], fabs(d__1)),
        ...
        if (accum) {
            ...
            if (!blk22 || incol < *ktop || ndcol > *kbot || ns <= 2) {
                ...
                for (jcol = min(ndcol, *kbot) + 1; i__4 < 0 ? jcol >= i__3 : jcol <= i__3;
                    ...
                    dlacpy_("ALL", &nu, &jlen, &wh[wh_offset], ldwh,
                ...
                for (jrow = jtop; i__3 < 0 ? jrow >= i__4 : jrow <= i__4; jrow += i__3) {
                    ...
                    jlen = min(i__5, i__7);
                    dgemm_("N", "N", &jlen, &nu, &nu, &c_b8, &h__[jrow + (incol + k1) * h_dim1],
                           ldh, &u[k1 + k1 * u_dim1], ldu, &c_b7, &wv[wv_offset], ldwv);
                ...
                for (jcol = min(ndcol, *kbot) + 1; i__3 < 0 ? jcol >= i__4 : jcol <= i__4;
                    ...
                    dlacpy_("ALL", &knz, &jlen, &h__[incol + 1 + j2 + jcol * h_dim1], ldh,
                            &wh[kzs + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply by U21' ==== */

                    dlaset_("ALL", &kzs, &jlen, &c_b7, &c_b7, &wh[wh_offset], ldwh);
                    dtrmm_("L", "U", "C", "N", &knz, &jlen, &c_b8, &u[j2 + 1 + (kzs + 1) * u_dim1],
                           ldu, &wh[kzs + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply top of H by U11' ==== */

                    dgemm_("C", "N", &i2, &jlen, &j2, &c_b8, &u[u_offset], ldu,
                           &h__[incol + 1 + jcol * h_dim1], ldh, &c_b8, &wh[wh_offset], ldwh);

                    /*                 ==== Copy top of H to bottom of WH ==== */

                    dlacpy_("ALL", &j2, &jlen, &h__[incol + 1 + jcol * h_dim1], ldh,
                            &wh[i2 + 1 + wh_dim1], ldwh);

                    /*                 ==== Multiply by U21' ==== */

                    dtrmm_("L", "L", "C", "N", &j2, &jlen, &c_b8, &u[(i2 + 1) * u_dim1 + 1], ldu,
                           &wh[i2 + 1 + wh_dim1], ldwh);
                    ...
                    i__5 = i4 - i2;
                    i__7 = j4 - j2;
                    dgemm_("C", "N", &i__5, &jlen, &i__7, &c_b8, &u[j2 + 1 + (i2 + 1) * u_dim1],
                           ldu, &h__[incol + 1 + j2 + jcol * h_dim1], ldh, &c_b8,
                           &wh[i2 + 1 + wh_dim1], ldwh);

                    /*                 ==== Copy it back ==== */

                    dlacpy_("ALL", &kdu, &jlen, &wh[wh_offset], ldwh,
                            &h__[incol + 1 + jcol * h_dim1], ldh);
                    /* L190: */
                }
                ...
                for (jrow = jtop; i__4 < 0 ? jrow >= i__3 : jrow <= i__3; jrow += i__4) {
                    ...
                    dlacpy_("ALL", &jlen, &knz, &h__[jrow + (incol + 1 + j2) * h_dim1], ldh,
                            &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U21 ==== */

                    dlaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[wv_offset], ldwv);
                    dtrmm_("R", "U", "N", "N", &jlen, &knz, &c_b8, &u[j2 + 1 + (kzs + 1) * u_dim1],
                           ldu, &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U11 ==== */

                    dgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &h__[jrow + (incol + 1) * h_dim1], ldh,
                           &u[u_offset], ldu, &c_b8, &wv[wv_offset], ldwv);

                    /*                 ==== Copy left of H to right of scratch ==== */

                    dlacpy_("ALL", &jlen, &j2, &h__[jrow + (incol + 1) * h_dim1], ldh,
                            &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

                    /*                 ==== Multiply by U21 ==== */

                    i__5 = i4 - i2;
                    dtrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(i2 + 1) * u_dim1 + 1], ldu,
                           &wv[(i2 + 1) * wv_dim1 + 1], ldwv);
                    ...
                    i__5 = i4 - i2;
                    i__7 = j4 - j2;
                    dgemm_("N", "N", &jlen, &i__5, &i__7, &c_b8,
                ...
                if (*wantz) {
                    ...
                    for (jrow = *iloz; i__3 < 0 ? jrow >= i__4 : jrow <= i__4; jrow += i__3) {
                        ...
                        dlacpy_("ALL", &jlen, &knz, &z__[jrow + (incol + 1 + j2) * z_dim1], ldz,
                                &wv[(kzs + 1) * wv_dim1 + 1], ldwv);

                        /*                    ==== Multiply by U12 ==== */

                        dlaset_("ALL", &jlen, &kzs, &c_b7, &c_b7, &wv[wv_offset], ldwv);
                        dtrmm_("R", "U", "N", "N", &jlen, &knz, &c_b8,
                               &u[j2 + 1 + (kzs + 1) * u_dim1], ldu, &wv[(kzs + 1) * wv_dim1 + 1],
                               ldwv);

                        /*                    ==== Multiply by U11 ==== */

                        dgemm_("N", "N", &jlen, &i2, &j2, &c_b8, &z__[jrow + (incol + 1) * z_dim1],
                               ldz, &u[u_offset], ldu, &c_b8, &wv[wv_offset], ldwv);

                        /*                    ==== Copy left of Z to right of scratch ==== */

                        dlacpy_("ALL", &jlen, &j2, &z__[jrow + (incol + 1) * z_dim1], ldz,
                                &wv[(i2 + 1) * wv_dim1 + 1], ldwv);

                        /*                    ==== Multiply by U21 ==== */

                        i__5 = i4 - i2;
                        dtrmm_("R", "L", "N", "N", &jlen, &i__5, &c_b8, &u[(i2 + 1) * u_dim1 + 1],
