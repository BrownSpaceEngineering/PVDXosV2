...
void uhf_main(void *pvParameters) {
    ...
    // spi_m_sync_set_mode(&SPI_0, SPI_MODE_0); //Not sure if needed
    gpio_set_pin_level(UHF_CS, false);

    // Initialize the UHF hardware
    status_t init_status = uhf_init(435E6);
...
status_t uhf_init(uint32_t frequency) {
    ...
    reg_result |= writeRegister(REG_LNA, reg_lna_value | 0x03);

    // set auto AGC
    reg_result |= writeRegister(REG_MODEM_CONFIG_3, 0x04);

    // set output power to 17 dBm
    // setTxPower(17); //TODO this seems complex and I haven't implemented it yet

    // put in standby mode
    reg_result |= writeRegister(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_STDBY);
